...
var borraRegistro = function (connection, tableName, numeroDocumento, callback) {
	...
	req.getConnection(function (err, connection) {
		...
		if(typeof connection === 'undefined'){
			...
			res.render('consulta', {usuario: req.user, page_title:"borrado de Victimas fallo", datavictima: null, message: "No se pudo establacer una conexion con la base de datos!"});
		} else {
		...
		req.getConnection(function (err, connection) {
			...
			if(typeof connection === 'undefined'){
				...
				var query = connection.query('Select * from victimas v, ' + TablaHecho + ' th WHERE v.Tipodocumento=th.Tipodocumento and v.Numerodocumento = th.Numerodocumento',
					...
					{
						res.render('listahecho', {usuario: req.user, page_title:"Lista de Victimas por Hecho Victimizante: " + TablaHecho, data: rows , message: null });
					};
